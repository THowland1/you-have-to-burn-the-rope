...
import { FRAME_HEIGHT, FRAME_WIDTH, c, canvas } from './canvas';
import { img } from './img';
...
class TitleSlide {
  ...
  constructor({ timeIn, timeOut, fadeTime }: ITitleSlide) {
    this.timeIn = timeIn;
  ...
  draw() {
    ...
    if (time - this.timeIn < this.fadeTime) {
      canvas.style.opacity = String((time - this.timeIn) / this.fadeTime);
    } else if (this.timeOut - time < this.fadeTime) {
      canvas.style.opacity = String((this.timeOut - time) / this.fadeTime);
    }
    c.textAlign = 'center';
...
class GameOverSlide {
  ...
  text: string;
  fadeTime: number;
  constructor({ timeIn, fadeTime, text }: IGameOverSlide) {
    this.timeIn = timeIn;
    this.fadeTime = fadeTime;
    this.text = text;
  }
...
class LyricSlide {
  ...
  constructor({ timeIn, timeOut, text }: ILyricSlide) {
    this.timeIn = timeIn;
    this.timeOut = timeOut;
    this.text = text;
  }
...
class ImageTextSlide {
  ...
  draw() {
    ...
    c.fillText(
      this.heading,
      offsetX + FRAME_WIDTH / 2,
      offsetY + FRAME_HEIGHT / 2 - 10
    );
    c.font = '400 12px Inter';
...
const slides = [
  new TitleSlide({ timeIn: 3.158386, timeOut: 10.540059, fadeTime: 0.45 }),
  new ImageTextSlide({
    heading: 'Design, Code, Graphics',
    lines: ['Kian Bashiri', '(mazapan.se)'],
    src: ASSET_URLS['../img/credits-1_200x200.png'],
    timeIn: 11.89449,
    timeOut: 19.374947,
    fadeTime: 0.45,
  }),
  new TextImageSlide({
    heading: 'Music',
    lines: ['Henrik Nåmark', '(reachground.se)'],
    src: ASSET_URLS['../img/credits-2_200x200.png'],
    timeIn: 20.633224,
    timeOut: 28.119119,
    fadeTime: 0.45,
  }),
  new ImageTextSlide({
    ...
    lines: ['Henrik Nåmark', 'Christian Dryden'],
    src: ASSET_URLS['../img/credits-3_200x200.png'],
    timeIn: 29.364552,
    timeOut: 36.967399,
    fadeTime: 0.45,
  }),
  new TextImageSlide({
    ...
    lines: ['Umami'],
    src: ASSET_URLS['../img/credits-4_200x200.png'],
    timeIn: 38.17481,
    timeOut: 45.787413,
    fadeTime: 0.45,
  }),
